...
The native collector classifies code editors as `DEVELOPMENT` with a `CODE` resource kind. This classification improvement adds no keystroke, clipboard, file-content, URL, or additional metadata collection. The separately documented local screenshot feature remains local-only; existing redaction and blocked-application rules still run before every upload. Failed inference attempts remain queued for retry and surface a temporary availability warning in the desktop.
