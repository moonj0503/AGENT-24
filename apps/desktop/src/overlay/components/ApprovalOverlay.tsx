...
export function ApprovalOverlay({ gap, actionId, onDecision, onOpenDetails }: { gap: GapData; actionId: string; onDecision: (actionId: string, status: "COMPLETED" | "REJECTED") => Promise<void>; onOpenDetails: () => void }) {
  ...
  return <section className="overlay-card" aria-labelledby="overlay-approval-title">
    ...
    <div className="overlay-actions"><button className="button primary" onClick={() => decide("COMPLETED")}>Approve</button><button className="button secondary" onClick={() => decide("REJECTED")}>Reject</button><button className="text-button" onClick={onOpenDetails}>Open full details</button></div>
  </section>;
}
