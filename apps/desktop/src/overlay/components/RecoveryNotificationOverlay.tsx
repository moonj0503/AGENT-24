...
export function RecoveryNotificationOverlay({ brief, onOpenDetails }: { brief: RecoveryBrief & { gapDurationSeconds?: number }; onOpenDetails: () => void }) {
  ...
  return <section className="overlay-card" aria-labelledby="overlay-recovery-title">
    ...
    <div className="overlay-actions"><button className="button primary" onClick={onOpenDetails}>Open Full Recovery Brief</button><button className="button secondary" onClick={dismissOverlayWithAnimation}>Resume</button></div>
  </section>;
}
