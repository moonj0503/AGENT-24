...
type Screen = "dashboard" | "goal" | "gap" | "recovery" | "permissions";

export function App() {
  const [screen, setScreen] = useState<Screen>("dashboard");
  const [inference, setInference] = useState<GoalInferenceResult>();
  const [goal, setGoal] = useState<GoalCandidate>();
  ...
  useEffect(() => { void loadInference(); void loadRulesData(); }, []);

  async function loadInference() {
    setBusy(true); setError(undefined);
    try { setInference(await fetchGoalInference()); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to load goals."); } finally { setBusy(false); }
  }

  async function loadRulesData() {
    try { setRules(await fetchPermissionRules()); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to load permissions."); }
  }

  async function beginGap() {
    setBusy(true); setError(undefined);
    try { setGap(await startGap()); setScreen("gap"); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to start Gap Mode."); } finally { setBusy(false); }
  }

  async function finishGap() {
    setBusy(true); setError(undefined);
    try { setBrief(await fetchRecoveryBrief()); setScreen("recovery"); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to prepare recovery."); } finally { setBusy(false); }
  }

  async function decideAction(actionId: string, status: "COMPLETED" | "REJECTED") {
    if (!gap) return;
    setGap(await updateAction(actionId, status));
  }

  const selectedPath = useMemo(() => goal?.suggestedGoalPath.join(" / ") ?? "No confirmed goal yet", [goal]);
  return <div className="app-shell">
    ...
    <main className="content"><header className="topbar"><div><p className="eyebrow">SATURDAY, AUGUST 1</p><h1>{screen === "dashboard" ? "Good morning, Won" : screen === "goal" ? "Confirm your goal" : screen === "gap" ? "Gap Mode" : screen === "recovery" ? "Welcome back" : "Permissions"}</h1></div><span className="demo-pill">DEMO MODE</span></header>
      {error && <div className="error" role="alert">{error}<button onClick={() => setError(undefined)}>Dismiss</button></div>}
      {screen === "dashboard" && <Dashboard busy={busy} inference={inference} goal={goal} path={selectedPath} onConfirm={() => setScreen("goal")} onStart={beginGap} />}
      ...
      {screen === "gap" && <GapView busy={busy} gap={gap} onAction={decideAction} onFinish={finishGap} />}
...
function GoalSelection({ result, selected, onSelect, onContinue }: { result?: GoalInferenceResult; selected?: GoalCandidate; onSelect: (id: string) => void; onContinue: () => void }) { return <section className="stack"><p className="lead">We inferred what you may be working on. Choose one to make it your confirmed goal.</p>{result?.candidates.map((candidate) => <button className={`goal-option ${selected?.candidateId === candidate.candidateId ? "selected" : ""}`} key={candidate.candidateId} onClick={() => onSelect(candidate.candidateId)}><span className="radio">{selected?.candidateId === candidate.candidateId ? "✓" : ""}</span><span><strong>{candidate.title}</strong><small>{candidate.description}</small><small className="evidence">Evidence: {candidate.evidence[0]?.description}</small></span><b>{Math.round(candidate.confidence * 100)}%</b></button>)}{!result && <EmptyState label="No goal candidates are available." />}{selected && <button className="button primary" onClick={onContinue}>Confirm this goal</button>}</section>; }
function GapView({ busy, gap, onAction, onFinish }: { busy: boolean; gap?: GapData; onAction: (id: string, status: "COMPLETED" | "REJECTED") => void; onFinish: () => void }) { return <section className="stack"><div className="gap-banner"><span className="pulse" /> Agent is preserving your workflow<h2>{gap?.plan.continuityObjective ?? "Preparing a continuity objective…"}</h2></div>{gap?.plan.actions.map((action) => <div className="action-row" key={action.actionId}><span className={`action-status ${action.status.toLowerCase()}`}>{action.status === "COMPLETED" ? "✓" : action.status === "WAITING_APPROVAL" ? "!" : "○"}</span><span><strong>{action.title}</strong><small>{action.reason}</small></span><span className="action-control">{action.status === "WAITING_APPROVAL" ? <><button onClick={() => onAction(action.actionId, "COMPLETED")}>Approve</button><button onClick={() => onAction(action.actionId, "REJECTED")}>Reject</button></> : action.status.toLowerCase()}</span></div>)}<button className="button primary" onClick={onFinish} disabled={busy}>{busy ? "Preparing recovery…" : "End Gap & view recovery"}</button></section>; }
...
function PermissionView({ rules }: { rules: PermissionRule[] }) { return <section className="stack"><p className="lead">The policy engine keeps actions reversible and visible. These demo rules apply to the agent.</p><div className="card">{rules.map((rule) => <div className="permission-row" key={rule.label}><span><strong>{rule.label}</strong><small>{rule.detail}</small></span><b className={`decision ${rule.decision.toLowerCase()}`}>{rule.decision}</b></div>)}</div></section>; }
