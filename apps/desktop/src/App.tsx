...
function PermissionView({ rules }: { rules: PermissionRule[] }) {
  const [blockedApplication, setBlockedApplication] = useState("");
  const [, refresh] = useState(0);
  const workflow = getDesktopObservationWorkflow();
  const status = workflow?.session.getSnapshot().status ?? "STOPPED";
  const blocked = workflow?.getState().privacy.blockedApplications ?? [];
  const update = async (action: () => Promise<void>) => { await action(); refresh((value) => value + 1); };
  return <section className="stack"><p className="lead">The policy engine keeps actions reversible and visible. Observation stores only sanitized activity metadata locally.</p>
    <div className="card"><div className="permission-row"><span><strong>Observation</strong><small>Current status: {status}</small></span>{status === "PAUSED" ? <button onClick={() => workflow && void update(() => workflow.resume())}>Resume</button> : <button onClick={() => workflow && void update(() => workflow.pause())}>Pause</button>}</div></div>
    <div className="card"><p className="eyebrow">BLOCKED APPLICATIONS</p><div className="permission-row"><input aria-label="Application identifier" value={blockedApplication} onChange={(event) => setBlockedApplication(event.target.value)} placeholder="process or application name" /><button onClick={() => workflow && void update(async () => { await workflow.addBlockedApplication(blockedApplication); setBlockedApplication(""); })}>Add</button></div>{blocked.map((identifier) => <div className="permission-row" key={identifier}><span>{identifier}</span><button onClick={() => workflow && void update(() => workflow.removeBlockedApplication(identifier))}>Remove</button></div>)}</div>
    <div className="card"><p className="eyebrow">LOCAL OBSERVATION DATA</p><div className="overlay-actions"><button className="button secondary" onClick={() => workflow && void workflow.clearLocalState(false)}>Clear observation history</button><button className="button secondary" onClick={() => workflow && void workflow.clearLocalState(true)}>Clear history and confirmed Goal</button></div></div>
    <div className="card">{rules.length ? rules.map((rule) => <div className="permission-row" key={rule.label}><span><strong>{rule.label}</strong><small>{rule.detail}</small></span><b className={`decision ${rule.decision.toLowerCase()}`}>{rule.decision}</b></div>) : <EmptyState label="No permission rules are available." />}</div>
  </section>;
}
