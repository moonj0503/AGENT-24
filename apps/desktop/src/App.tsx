...
import { fetchGapHistory } from "./features/history/api";
import type { GapData } from "./features/gap/api";
import { useDesktopWorkflowState } from "./features/workflow/store";
...
function HistoryView() { const [history, setHistory] = useState<Awaited<ReturnType<typeof fetchGapHistory>>>(); const [error, setError] = useState<string>(); useEffect(() => { void fetchGapHistory().then(setHistory).catch((cause) => setError(messageOf(cause, "Unable to load history."))); }, []); if (error) return <div className="error">{error}</div>; if (!history) return <EmptyState label="Loading history…" />; return <section className="stack"><p className="lead">Completed and active Gap sessions from the backend.</p>{history.items.length ? history.items.map(({ gapSession, recoveryBrief }) => <div className="card" key={gapSession.gapId}><strong>{recoveryBrief?.goalBeforeGap ?? gapSession.goalId}</strong><p>{gapSession.status} · {new Date(gapSession.startedAt).toLocaleString()}</p><small>{recoveryBrief ? `${recoveryBrief.completedActions.length} completed · ${recoveryBrief.pendingActions.length} pending` : "Recovery not generated yet"}</small></div>) : <EmptyState label="No Gap history yet." />}</section>; }
