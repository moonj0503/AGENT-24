...
import { RawApiChat } from "./features/chat/RawApiChat";

type Screen = "dashboard" | "chat" | "goal" | "gap" | "recovery" | "permissions" | "history";
...
export function App() {
  ...
  async function cancelGapSetup() {
    const observation = getDesktopObservationWorkflow();
    if (!observation) { setError("Goal identification is not ready."); return; }
    setError(undefined);
    try {
      await observation.cancelGapMode();
      setScreen("dashboard");
    } catch (cause) {
      setError(messageOf(cause, "Unable to cancel Gap setup."));
    }
  }
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">C</span><span>Continuity<br /><strong>Agent</strong></span></div><p className="eyebrow">YOUR WORKSPACE</p>
      <NavItem active={screen === "dashboard"} onClick={() => setScreen("dashboard")}>Dashboard</NavItem><NavItem active={screen === "chat"} onClick={() => setScreen("chat")}>API Chat</NavItem><NavItem active={screen === "gap"} onClick={() => setScreen("gap")}>Gap Mode</NavItem><NavItem active={screen === "recovery"} onClick={() => setScreen("recovery")}>Recovery</NavItem><NavItem active={screen === "history"} onClick={() => setScreen("history")}>History</NavItem>
...
function screenTitle(screen: Screen) { return screen === "dashboard" ? "Continuity workspace" : screen === "chat" ? "API Chat" : screen === "goal" ? "Confirm your goal" : screen === "gap" ? "Gap Mode" : screen === "recovery" ? "Welcome back" : screen === "history" ? "History" : "Permissions"; }
