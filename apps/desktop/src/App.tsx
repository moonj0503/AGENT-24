...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="content"><header className="topbar"><div><p className="eyebrow">SATURDAY, AUGUST 1</p><h1>{screenTitle(screen)}</h1></div><span className="demo-pill">DEMO MODE</span></header>
      ...
      {screen === "dashboard" && <Dashboard busy={busy} inference={inference} goal={goal} path={selectedPath} onConfirm={requestGoalConfirmation} onStart={requestGapStart} />}
    ...
    {!isNativeOverlayAvailable() && overlay.state === "GAP_START_CONFIRMATION" && <div className="main-overlay-backdrop"><GapStartOverlay goal={overlay.selectedGoal ?? goal} gap={overlay.gap} onConfirm={confirmGapStart} onOpenDetails={() => { dismissOverlay(); setScreen("gap"); }} /></div>}
