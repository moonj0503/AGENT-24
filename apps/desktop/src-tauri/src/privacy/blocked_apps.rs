const BLOCKED_APPLICATIONS: &[&str] = &[
    "1password", "authy", "bitwarden", "dashlane", "google authenticator", "keeper", "lastpass",
...
pub fn is_blocked(application_name: &str, window_title: &str) -> bool {
    ...
    BLOCKED_APPLICATIONS.iter().any(|blocked| application_name.contains(blocked))
        || PRIVATE_WINDOW_MARKERS.iter().any(|marker| window_title.contains(marker))
}
